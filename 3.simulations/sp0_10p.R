...
doe = readRDS(file = "2.get-doe/doe/test_10p.rds")
...
run_model = function(par, names, ...) {
  ...
  # output directory
  output_dir = "output_Yansong"
  
  # Yansong: change config
  conf[grepl("species\\.k\\.sp", names(conf))] <- par[grepl("species\\.K\\.sp", names(par))]
  
  conf[grepl("species\\.k\\.sp", names(conf))] <- rep(1.01,16)
  ...
  conf_unlist <- unlist(conf)
  write_osmose(conf,file = file.path(config_dir, "modified_config.csv"),sep = ",")
  
  # run Osmose Model
  # 检查校准参数文档中的内容是否已经迁移
  new_config = read.csv(file = file.path(config_dir, "modified_config.csv"), header=FALSE, stringsAsFactors = FALSE, na.strings = c(""))
  run_osmose(input = config_file, output = output_dir, osmose = jar_file, version = "4.4.0")
